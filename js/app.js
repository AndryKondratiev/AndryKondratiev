(() => {
    ...
    window.onload = function() {
        const parallax = document.querySelectorAll(".examples");
        if (parallax) {
            const image = document.querySelectorAll(".examples__image");
            for (let index = 0; index < parallax.length; index++) {
                ...
                function setMouseParalaxStyle() {
                    ...
                    image[index].style.cssText = `transform: translate(${positionX / forImageX}%, ${positionY / forImageY}%);`;
                    requestAnimationFrame(setMouseParalaxStyle);
                }
                setMouseParalaxStyle();
                parallax[index].addEventListener("mousemove", (function(e) {
                    const parallaxWidth = parallax[index].offsetWidth;
                    const parallaxHeight = parallax[index].offsetHeight;
                    const coordX = e.pageX - parallaxWidth / 2;
                    const coordY = e.pageY - parallaxHeight / 2;
                    coordXprocent = coordX / parallaxWidth * 100;
                    coordYprocent = coordY / parallaxHeight * 100;
                }));
                parallax[index].addEventListener("mouseout", (function(e) {
                    coordXprocent = 0;
                    coordYprocent = 0;
                }));
            }
    ...
    document.addEventListener("DOMContentLoaded", (() => {
        const animItems = document.querySelectorAll("[data-ScrollAnimItem]");
        if (animItems.length > 0) {
            ...
            function animOnScroll() {
                for (let index = 0; index < animItems.length; index++) {
                    ...
                    const animStart = animItem.dataset.scrollanimitem ? parseInt(animItem.dataset.scrollanimitem) : 50;
                    let animItemPoint = window.innerHeight - animItemHeight / 100 * -animStart;
